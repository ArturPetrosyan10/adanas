...
// let datepicker = $('.fs-datepicker-body');
//
...
// function changedCartRow(productId, count) {
//     let hiddenInput = $('#changed-cart-data').find('input');
//     let currentObj = hiddenInput.val();
//
//     if (currentObj !== '') {
//         let changedValArr = currentObj.split('; ');
//         let switcher = false;
//
//         for (let i = 0; i < changedValArr.length; i++) {
//             if (changedValArr[i] === '') {
//                 changedValArr.splice(i, 1);
//                 continue;
//             }
//             if (changedValArr[i].split(': ')[0] === productId) {
//                 if (changedValArr[i].split(': ')[1] === count) {
//                     changedValArr.splice(i, 1);
//                     continue;
//                 }
//                 switcher = true;
//                 changedValArr[i] = changedValArr[i].split(': ')[0] + ': ' + count;
//             }
//         }
//
//         if (switcher === false) {
//             changedValArr.push(productId + ': ' + count);
//         }
//
//         console.log(changedValArr);
//     } else {
//         hiddenInput.val(productId + ': ' + count + '; ');
//     }
// }
...
$('.fs-show-unread').click(function() {
    $.post("http://" + window.location.hostname + "/sadmin/engine.php", {
            ...
            if (data === 'empty') {
                ...
                for (let i = 0; i < dataArr.length; i++) {
                    ...
                    target.append('<li class="fs-notification-list-el new-notification" data-id="' + dataArr[i].id + '">\n' +
                        ...
                        '                                </label>\n' +
                        '                                <a href="' + dataArr[i].link + '" class="fs-notification-link">\n' +
                        '                                    <div class="fs-notification-thumbnail">\n' +
                        '                                        <img src="' + dataArr[i].thumbnail + '" alt="" />\n' +
                        '                                    </div>\n' +
                        '                                    <div class="fs-notification-data">\n' + dataArr[i].message +
                        '                                        <time class="fs-notification-date">' + dataArr[i].date + '</time>\n' +
                        '                                    </div>\n' +
                        '                                </a>\n' +
                        '                            </li>');
                }
...
$('.fs-show-all-notif').click(function() {
    $.post("LINK", {
        role: 'getAllNotifications'
    }).done(function(data) {
        data = [{
            'message': '<h4><strong>Կազմակերպության անվանում</strong> ունի նոր ապրանքներ լոռեմ իպսում դոլոռ սիթ ամեթ, իդքուե ֆաբուլաս ինսթո լոռեմ իպսում դոլոռ սիթ ամեթ, իդքուե ֆաբուլաս ինսթո</h4>',
            'link': '#sss',
            'date': '35 րոպե առաջ',
            "id": "0114",
            'thumbnail': 'assets/media/images/notification-thumbnail.jpg',
            'new': true
        },
            {
                'message': '<h4><strong>Կազմակերպության անվանում</strong> ունի նոր ապրանքներ լոռեմ իպսում դոլոռ սիթ ամեթ, իդքուե ֆաբուլաս ինսթո լոռեմ իպսում դոլոռ սիթ ամեթ, իդքուե ֆաբուլաս ինսթո</h4>',
...
$('.fs-cart-supplier-submit-button').click(function() {
    let allRows = $('.fs-cart-supplier-row');
    let switcher = false;
    let arr = [];
    for (let i = 0; i < allRows.length; i++) {
        let productId = parseInt(allRows.eq(i).attr('data-id'));
        let productCount = parseInt(allRows.eq(i).find('.fs-cart-piece-prod-count').value());
        let productCurrentCount = parseInt(allRows.eq(i).find('.fs-cart-piece-prod-calc-block').attr('data-current-count'));
        if (productCount !== productCurrentCount) {
            switcher = true;
            let string = productId + "|" + productCount;
            arr.push(string);
        }
    }
    arr = arr.join(' ');
...
$('body').click('.fs-to-top-button',function() {
    $("html, body").animate({
        scrollTop: 0
    }, "slow");
    return false;
...
$('.fs-cart-supplier-checkbox').click(function() {
    ...
    let price = parseInt($(this).closest('.fs-cart-supplier-block').find('.fs-cart-supplier-stock-total-price').text().replace(/\D/g, ''));
    $('.fs-cart-aside-grand-total-price-span').text(parseInt($('.fs-cart-aside-grand-total-price-span').text()) + price);
})
...
$(document).on('click', '.saveTemplate', function() {
    ...
    if (ids.length < 1) {
        ...
        $.post("/site/save-template", {
            ...
            window.location.href = '/cart?save=success';
        });
    }
});
...
$(document).on('click', '.sendOrderRequest', function() {
    let data = {};
    let price = {};
    let delivery_date = {};
...
$(document).on('click', '.fs-cart-product-remove', function() {
    $('#basket-remove-el-dialog').attr('data-id', $(this).closest('.fs-cart-supplier-row').attr('data-id'));
    $('#basket-remove-el-dialog').addClass('active');
});
