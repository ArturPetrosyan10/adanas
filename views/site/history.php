    ...
    <div class="fs-breadcrumbs-wrapper">
        <div class="fs-container">
            <ul class="fs-breadcrumbs-list">
                ...
                <?php } ?>
                <li class="fs-breadcrumbs-el"><?= $GLOBALS['text']['__orders__'] ?></li>
            </ul>
        </div>
    </div>
            ...
            <div class="fs-personal-body">
                ...
                <div class="fs-personal-orders-wrapper">
                    <div class="fs-personal-order-tabs">
                        <a href="/personal-history" class="fs-personal-order-tab <?= !$_GET['state'] ? 'active' : ''?>"><?= $GLOBALS['text']["_all_"] ?></a>
                        <a href="/personal-history?state=1" class="fs-personal-order-tab <?= ($_GET['state'] && $_GET['state'] == 1) ? 'active' : ''?>"><i class="fs-icon-ordered"></i><?= $GLOBALS['text']['__ordered__'] ?></a>
                        <a href="/personal-history?state=2" class="fs-personal-order-tab <?= ($_GET['state'] && $_GET['state'] == 2) ? 'active' : ''?>"><i class="fs-icon-approved-with-change"></i><?= $GLOBALS['text']['__sent_for_confirmation__'] ?></a>
                        <a href="/personal-history?state=3" class="fs-personal-order-tab <?= ($_GET['state'] && $_GET['state'] == 3) ? 'active' : ''?>"><i class="fs-icon-approved"></i><?= $GLOBALS['text']['__acepted__'] ?></a>
                        <a href="/personal-history?state=4" class="fs-personal-order-tab <?= ($_GET['state'] && $_GET['state'] == 4) ? 'active' : ''?>"><i class="fs-icon-ordered"></i><?= $GLOBALS['text']['__closed__'] ?></a>
                        <a href="/personal-history?state=5" class="fs-personal-order-tab <?= ($_GET['state'] && $_GET['state'] == 5) ? 'active' : ''?>"><i class="fs-icon-rejected"></i><?= $GLOBALS['text']['__rejected__'] ?></a>
                    </div>
                    <div class="fs-category-sort-row mobile-only">
                        <select name="type" id="drop_mobile" class="fs-dropdown">
                            ...
                            <div class="fs-double-datepicker-body">
                                <div class="fs-double-datepicker-row">
                                    <div class="fs-double-datepicker-col">
                                        <input type="text" name="fromdate" value="" class="fs-set-date-from"  placeholder="<?php $GLOBALS['text']['__choose__'] ?>" />
                                        <i class="fs-icon-calendar"></i>
                                    </div>
                                    <div class="fs-double-datepicker-col">
                                        <input type="text" name="todate" value="" class="fs-set-date-to"  placeholder="<?php $GLOBALS['text']['__choose__'] ?>" />
                                        <i class="fs-icon-calendar"></i>
                                    </div>
                                </div>
                            ...
                            <?php foreach ($orders as $order): ?>
                                <div class="fs-personal-order-table-body-row">
                                    ...
                                    <div class="fs-personal-order-table-col order">
                                        <a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label">N<?= str_pad($order->id, 9, "0", STR_PAD_LEFT) ?></a>
                                    </div>
                                    <div class="fs-personal-order-table-col date"><a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label"><time><?= date('d.m.Y', strtotime($order->created_at)) ?></time></a></div>
                                    <div class="fs-personal-order-table-col supplier">
                                        <a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label">
                                            <h5 class="fs-personal-order-supplier-name" style="text-align: center;"><?= $order->seller->legal_name ?> </h5>
                                        </a>
                                    </div>
                                    <div class="fs-personal-order-table-col status"><a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label"><i class="<?= $status[$order->status]['icon'] ?>" <?php if($status[$order->status]['icon'] == "fs-icon-approved-with-change"){ echo 'style="color: #297551;"';}?>></i><?= $status[$order->status]['text'] ?></a></div>
                                    <div class="fs-personal-order-table-col result"><a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label"><?= number_format($order->price, '0', ' ', ',') ?> <?= $GLOBALS['text']['__dr__'] ?></a></div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <?php } else { ?>
                            <div class="fs-personal-order-table">
                                ...
                                <?php foreach ($orders as $order): ?>
                                    <div class="fs-personal-order-table-body-row " >
                                        ...
                                        <div class="fs-personal-order-table-col order ">
                                            <a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label">N<?= str_pad($order->id, 9, "0", STR_PAD_LEFT) ?> <time style="float:right;"><?= date('d.m.Y', strtotime($order->created_at)) ?></time></a>
                                            <br>
                                            <div><?= $order->seller->legal_name ?></div>
                                            <a href="/personal-history-details?id=<?= $order->id ?>" style="justify-content: left;padding:5px 0px;" class="fs-personal-order-table-order-label fs-personal-order-tab"><i style="margin-right:10px;" class="<?= $status[$order->status]['icon'] ?>" <?php if($status[$order->status]['icon'] == "fs-icon-approved-with-change"){ echo 'style="color: #297551;"';}?>></i><?= $status[$order->status]['text'] ?></a>

                                            <div class="result"><a href="/personal-history-details?id=<?= $order->id ?>" class="fs-personal-order-table-order-label"><?= $GLOBALS['text']['__total__'] ?>՝ <b style="float:right;"><?= number_format($order->price, '0', ' ', ',') ?> <?= $GLOBALS['text']['__dr__'] ?> </b></a></div>
