...
<?php if($product->send_notice == 1 && strtotime(date('Y-m-d'))<strtotime( date("Y-m-d", strtotime("+10 days", strtotime($product->create_date))))){
    $data_new = "Նոր" ;
    $class = 'fs-new-product';
 }else {
    if($discount->discount_procent){
        $class = 'fs-new-product';
        $data_new = $discount->discount_procent.'%';
    }
 } ?>
    <article class="fs-product-card <?= $class ?>" data-new="<?= $data_new ?>">
...
<?php } else { ?>
        <span class="fs-product-current-price" data-price-cur="<?= $GLOBALS['text']['__dr__'] ?>">
            ...
            <?= number_format($product->price - ($product->price*$discount->discount_procent/100), 0);?>
        </span>
<?php } ?>
